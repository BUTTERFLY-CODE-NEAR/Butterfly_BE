...
import com.codenear.butterfly.payment.kakaoPay.domain.dto.handler.ApproveFreePaymentHandler;
import com.codenear.butterfly.payment.kakaoPay.domain.dto.handler.ApproveHandler;
...
public class SinglePaymentService {
    ...
    public void approveResponse(String pgToken, Long memberId) {
        ...
        int usePoint = parsingStringToInt(memberId, KakaoPayRedisField.POINT.getFieldName());
    ...
    public void cancelPayment(Long memberId, String productName, int quantity) {
        restoreQuantity(productName, quantity, kakaoPaymentRedisRepository.getHashFieldValue(memberId, KakaoPayRedisField.ORDER_ID.getFieldName()));
    ...
    private void addOrderTypeDetails(OrderDetails orderDetails,
        ...
        switch (orderType) {
            case PICKUP -> {
                String pickupPlace = kakaoPaymentRedisRepository.getHashFieldValue(memberId, KakaoPayRedisField.PICKUP_PLACE.getFieldName());
                LocalDate pickupDate = LocalDate.parse(kakaoPaymentRedisRepository.getHashFieldValue(memberId, KakaoPayRedisField.PICKUP_DATE.getFieldName()));
                LocalTime pickupTime = LocalTime.parse(kakaoPaymentRedisRepository.getHashFieldValue(memberId, KakaoPayRedisField.PICKUP_TIME.getFieldName()));
            ...
            case DELIVER -> {
                ...
                LocalDate deliverDate = LocalDate.parse(kakaoPaymentRedisRepository.getHashFieldValue(memberId, KakaoPayRedisField.DELIVER_DATE.getFieldName()));
